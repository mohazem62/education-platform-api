...
{
    public sealed record AssignmentRequest(Guid TeacherId, Guid StudentId, Guid SubjectId);
    [HttpPost] public async Task<IActionResult> Create(AssignmentRequest r, CancellationToken ct) { if (!await db.TeacherSubjects.AnyAsync(x => x.TeacherId == r.TeacherId && x.SubjectId == r.SubjectId, ct) || !await db.StudentSubjects.AnyAsync(x => x.StudentId == r.StudentId && x.SubjectId == r.SubjectId, ct)) throw new AppException(409, ErrorCodes.Validation, "المادة ليست مشتركة بين الطالب والمعلم."); var x = new TeacherStudentAssignment { TeacherId = r.TeacherId, StudentId = r.StudentId, SubjectId = r.SubjectId, AssignedAt = DateTimeOffset.UtcNow }; db.Add(x); await db.SaveChangesAsync(ct); return StatusCode(201, ApiResponse<object>.Ok(new { x.Id })); }
    [HttpGet] public async Task<IActionResult> List([FromQuery] Guid? teacherId, [FromQuery] Guid? studentId, CancellationToken ct) { var q = db.TeacherStudentAssignments.AsNoTracking(); if (teacherId.HasValue) q = q.Where(x => x.TeacherId == teacherId); if (studentId.HasValue) q = q.Where(x => x.StudentId == studentId); return Ok(ApiResponse<object>.Ok(await q.Take(100).Select(x => new { x.Id, x.TeacherId, x.StudentId, x.SubjectId, x.AssignedAt }).ToListAsync(ct))); }
