...
try
{
    ...
    builder.Services.AddEndpointsApiExplorer(); builder.Services.AddSwaggerGen(o => { o.TagActionsBy(SwaggerTagging.For); o.DocumentFilter<SwaggerTagOrderFilter>(); o.SwaggerDoc("v1", new OpenApiInfo { Title = "منصة التعليم - API", Version = "v1", Description = "واجهة عربية أولًا لإدارة التعليم والعمليات المالية" }); o.CustomSchemaIds(t => (t.FullName ?? t.Name).Replace('+', '.')); o.AddSecurityDefinition("Bearer", new OpenApiSecurityScheme { Type = SecuritySchemeType.Http, Scheme = "bearer", BearerFormat = "JWT", Description = "أدخل JWT الناتج من /api/v1/auth/login" }); o.AddSecurityRequirement(new OpenApiSecurityRequirement { [new OpenApiSecurityScheme { Reference = new OpenApiReference { Type = ReferenceType.SecurityScheme, Id = "Bearer" } }] = [] }); });
