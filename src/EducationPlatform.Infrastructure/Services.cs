...
public sealed class JwtOptions { public const string Section = "Jwt"; public string Issuer { get; set; } = "EducationPlatform"; public string Audience { get; set; } = "EducationPlatform.Frontend"; public string SigningKey { get; set; } = string.Empty; public int AccessTokenMinutes { get; set; } = 15; public int RefreshTokenDays { get; set; } = 30; }
...
public sealed class AuthService(AppDbContext db, UserManager<ApplicationUser> users, IOptions<JwtOptions> jwtOptions, IDateTimeProvider clock, ICurrentUser current) : IAuthService
{
    ...
    private async Task<AuthResponse> IssueAsync(ApplicationUser user, string? device, CancellationToken ct, bool save = true)
    {
        ...
        var expires = clock.UtcNow.AddMinutes(_jwt.AccessTokenMinutes);
...
public sealed class StudentService(AppDbContext db, UserManager<ApplicationUser> users, ICurrentUser current, IDateTimeProvider clock) : IStudentService
{
    ...
    public async Task<StudentResponse> GetAsync(Guid id, CancellationToken ct) => Map(await db.Students.AsNoTracking().SingleOrDefaultAsync(x => x.Id == id, ct) ?? throw new AppException(404, ErrorCodes.StudentNotFound, "الطالب غير موجود."));
    public async Task<StudentResponse> CreateAsync(CreateStudentRequest r, CancellationToken ct)
    {
        ...
        db.Students.Add(entity); await db.AuditAsync(current, "StudentCreated", nameof(Student), entity.Id, null, r.FullName, ct); await db.SaveChangesAsync(ct); await tx.CommitAsync(ct); return Map(entity);
    }
    public async Task<StudentResponse> UpdateAsync(Guid id, UpdateStudentRequest r, CancellationToken ct)
    {
        ...
        await db.AuditAsync(current, "StudentUpdated", nameof(Student), id, null, r.FullName, ct); await db.SaveChangesAsync(ct); return Map(x);
    }
...
{
    public async Task<PageResult<TeacherResponse>> SearchAsync(PageRequest p, Guid? subjectId, Guid? curriculumId, CancellationToken ct) { var q = db.Teachers.AsNoTracking(); if (!string.IsNullOrWhiteSpace(p.Search)) q = q.Where(x => x.FullName.Contains(p.Search) || x.PhoneNumber.Contains(p.Search)); if (subjectId.HasValue) q = q.Where(x => x.Subjects.Any(y => y.SubjectId == subjectId)); if (curriculumId.HasValue) q = q.Where(x => x.Curricula.Any(y => y.CurriculumId == curriculumId)); var n = Math.Max(1, p.PageNumber); var z = Math.Clamp(p.PageSize, 1, 100); var count = await q.CountAsync(ct); var data = await q.OrderBy(x => x.FullName).Skip((n - 1) * z).Take(z).Select(x => Map(x, false)).ToListAsync(ct); return new(data, n, z, count); }
    public async Task<TeacherResponse> GetAsync(Guid id, CancellationToken ct) => Map(await Required(id, ct), true);
    public async Task<TeacherResponse> CreateAsync(CreateTeacherRequest r, CancellationToken ct) { await using var tx = await db.Database.BeginTransactionAsync(ct); var phone = PhoneNormalizer.Normalize(r.PhoneNumber); var user = new ApplicationUser { UserName = r.UserName, DisplayName = r.FullName, PhoneNumber = phone }; var result = await users.CreateAsync(user, r.Password); if (!result.Succeeded) throw new AppException(400, ErrorCodes.Validation, string.Join(" ", result.Errors.Select(x => x.Description))); await users.AddToRoleAsync(user, Roles.Teacher); var x = new Teacher { UserId = user.Id, FullName = r.FullName, PhoneNumber = phone, WhatsApp = r.WhatsApp, DefaultPerSessionRate = r.DefaultPerSessionRate, PreferredPayoutMethod = r.PreferredPayoutMethod, EWalletNumber = r.EWalletNumber, InstaPayIdentifier = r.InstaPayIdentifier, Subjects = r.SubjectIds.Distinct().Select(id => new TeacherSubject { SubjectId = id }).ToList(), Curricula = r.CurriculumIds.Distinct().Select(id => new TeacherCurriculum { CurriculumId = id }).ToList() }; db.Teachers.Add(x); await db.AuditAsync(current, "TeacherCreated", nameof(Teacher), x.Id, null, r.FullName, ct); await db.SaveChangesAsync(ct); await tx.CommitAsync(ct); return Map(x, true); }
    public async Task<TeacherResponse> UpdateAsync(Guid id, UpdateTeacherRequest r, CancellationToken ct) { var x = await Required(id, ct); if (!Enum.TryParse<AccountStatus>(r.Status, true, out var status)) throw new AppException(422, ErrorCodes.Validation, "حالة الحساب غير صالحة."); x.FullName = r.FullName; x.PhoneNumber = PhoneNormalizer.Normalize(r.PhoneNumber); x.WhatsApp = r.WhatsApp; x.DefaultPerSessionRate = r.DefaultPerSessionRate; x.PreferredPayoutMethod = r.PreferredPayoutMethod; x.EWalletNumber = r.EWalletNumber; x.InstaPayIdentifier = r.InstaPayIdentifier; x.Status = status; x.UpdatedAt = clock.UtcNow; db.TeacherSubjects.RemoveRange(await db.TeacherSubjects.Where(s => s.TeacherId == id).ToListAsync(ct)); db.TeacherCurricula.RemoveRange(await db.TeacherCurricula.Where(s => s.TeacherId == id).ToListAsync(ct)); db.TeacherSubjects.AddRange(r.SubjectIds.Distinct().Select(subjectId => new TeacherSubject { TeacherId = id, SubjectId = subjectId })); db.TeacherCurricula.AddRange(r.CurriculumIds.Distinct().Select(curriculumId => new TeacherCurriculum { TeacherId = id, CurriculumId = curriculumId })); await db.AuditAsync(current, "TeacherUpdated", nameof(Teacher), id, null, r.FullName, ct); await db.SaveChangesAsync(ct); return Map(x, true); }
    ...
    private static TeacherResponse Map(Teacher x, bool sensitive) => new(x.Id, x.FullName, x.PhoneNumber, x.WhatsApp, x.DefaultPerSessionRate, x.Status.ToString(), sensitive ? PhoneNormalizer.Mask(x.EWalletNumber ?? x.InstaPayIdentifier) : null);
}
...
public sealed class SessionService(AppDbContext db, ICurrentUser current, IDateTimeProvider clock) : ISessionService
{
    ...
    {
        var teacher = await db.Teachers.SingleOrDefaultAsync(x => x.Id == r.TeacherId, ct) ?? throw new AppException(404, ErrorCodes.TeacherNotFound, "المعلم غير موجود.");
        if (!await db.Students.AnyAsync(x => x.Id == r.StudentId, ct)) throw new AppException(404, ErrorCodes.StudentNotFound, "الطالب غير موجود.");
        if (!await db.TeacherStudentAssignments.AnyAsync(x => x.TeacherId == r.TeacherId && x.StudentId == r.StudentId && x.SubjectId == r.SubjectId, ct)) throw new AppException(409, ErrorCodes.Validation, "المعلم غير مكلّف بهذا الطالب والمادة.");
        var x = new ClassSession { StudentId = r.StudentId, TeacherId = r.TeacherId, SubjectId = r.SubjectId, ScheduledAt = r.ScheduledAt.ToUniversalTime(), DurationMinutes = r.DurationMinutes, ClassLink = r.ClassLink, StudentCreditCost = r.StudentCreditCost, TeacherRateSnapshot = teacher.DefaultPerSessionRate };
        db.Sessions.Add(x); await db.SaveChangesAsync(ct); return Map(x);
    }
    ...
    private static SessionResponse Map(ClassSession x) => new(x.Id, x.StudentId, x.TeacherId, x.SubjectId, x.ScheduledAt, x.DurationMinutes, x.ClassLink, x.Status.ToString(), x.AttendanceStatus?.ToString(), x.TeacherRateSnapshot, x.StudentCreditCost);
}
