...
public sealed class AuthService(AppDbContext db, UserManager<ApplicationUser> users, IOptions<JwtOptions> jwtOptions, IDateTimeProvider clock, ICurrentUser current) : IAuthService
{
    ...
    {
        var user = await RequiredCurrentAsync();
        return new(user.Id, user.UserName!, user.DisplayName, user.Email, (await users.GetRolesAsync(user)).ToArray());
    }
    ...
    private async Task<AuthResponse> IssueAsync(ApplicationUser user, string? device, CancellationToken ct, bool save = true)
    {
        ...
        var currentUser = new CurrentUserResponse(user.Id, user.UserName!, user.DisplayName, user.Email, roles.ToArray());
...
{
    public async Task<PageResult<TeacherResponse>> SearchAsync(PageRequest p, Guid? subjectId, Guid? curriculumId, CancellationToken ct) { var q = db.Teachers.AsNoTracking(); if (!string.IsNullOrWhiteSpace(p.Search)) q = q.Where(x => x.FullName.Contains(p.Search) || x.PhoneNumber.Contains(p.Search)); if (subjectId.HasValue) q = q.Where(x => x.Subjects.Any(y => y.SubjectId == subjectId)); if (curriculumId.HasValue) q = q.Where(x => x.Curricula.Any(y => y.CurriculumId == curriculumId)); var n = Math.Max(1, p.PageNumber); var z = Math.Clamp(p.PageSize, 1, 100); var count = await q.CountAsync(ct); var data = await q.OrderBy(x => x.FullName).Skip((n - 1) * z).Take(z).Select(x => Map(x, false, null, null, null)).ToListAsync(ct); return new(data, n, z, count); }
    public async Task<TeacherResponse> GetAsync(Guid id, CancellationToken ct)
    {
        var teacher = await Required(id, ct);
        var subjects = await (from link in db.TeacherSubjects.AsNoTracking() join item in db.Subjects.AsNoTracking() on link.SubjectId equals item.Id where link.TeacherId == id orderby item.NameAr select new LookupResponse(item.Id, item.NameAr, item.NameEn, item.Code)).ToListAsync(ct);
        var curricula = await (from link in db.TeacherCurricula.AsNoTracking() join item in db.Curricula.AsNoTracking() on link.CurriculumId equals item.Id where link.TeacherId == id orderby item.NameAr select new LookupResponse(item.Id, item.NameAr, item.NameEn, item.Code)).ToListAsync(ct);
        return Map(teacher, true, await RateRows(id, ct), subjects, curricula);
    }
    ...
    private static TeacherResponse Map(Teacher x, bool sensitive, IReadOnlyList<TeacherStageRateResponse>? rates = null, IReadOnlyList<LookupResponse>? subjects = null, IReadOnlyList<LookupResponse>? curricula = null) => new(x.Id, x.FullName, x.PhoneNumber, x.WhatsApp, x.DefaultPerSessionRate, x.Status.ToString(), sensitive ? PhoneNormalizer.Mask(x.EWalletNumber ?? x.InstaPayIdentifier) : null, x.DefaultCurrency, rates, subjects, curricula);
