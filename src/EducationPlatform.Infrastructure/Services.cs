...
public sealed class TeacherService(AppDbContext db, UserManager<ApplicationUser> users, ICurrentUser current, IDateTimeProvider clock) : ITeacherService
{
    ...
    private static TeacherResponse Map(Teacher x, bool sensitive, IReadOnlyList<TeacherStageRateResponse>? rates = null) => new(x.Id, x.FullName, x.PhoneNumber, x.WhatsApp, x.DefaultPerSessionRate, x.Status.ToString(), sensitive ? PhoneNormalizer.Mask(x.EWalletNumber ?? x.InstaPayIdentifier) : null, x.ZoomMeetingUrl, x.DefaultCurrency, rates);
...
public sealed class SessionService(AppDbContext db, ICurrentUser current, IDateTimeProvider clock) : ISessionService
{
    public async Task<SessionResponse> CreateAsync(CreateSessionRequest r, CancellationToken ct)
    {
        ...
        var student = await db.Students.SingleOrDefaultAsync(x => x.Id == r.StudentId, ct) ?? throw new AppException(404, ErrorCodes.StudentNotFound, "الطالب غير موجود.");
        var assignment = await db.TeacherStudentAssignments.SingleOrDefaultAsync(x => x.TeacherId == r.TeacherId && x.StudentId == r.StudentId && x.SubjectId == r.SubjectId, ct) ?? throw new AppException(409, ErrorCodes.Validation, "المعلم غير مكلّف بهذا الطالب والمادة.");
        var stageRate = await db.TeacherGradeRates.Where(x => x.TeacherId == r.TeacherId && x.GradeLevelId == student.GradeLevelId).Select(x => new { x.Rate, x.Currency }).SingleOrDefaultAsync(ct);
        var x = new ClassSession { StudentId = r.StudentId, TeacherId = r.TeacherId, SubjectId = r.SubjectId, ScheduledAt = r.ScheduledAt.ToUniversalTime(), DurationMinutes = r.DurationMinutes, ClassLink = r.ClassLink ?? teacher.ZoomMeetingUrl, StudentCreditCost = r.StudentCreditCost, TeacherRateSnapshot = stageRate?.Rate ?? teacher.DefaultPerSessionRate, TeacherRateCurrencySnapshot = stageRate?.Currency ?? teacher.DefaultCurrency, StudentPriceSnapshot = assignment.SessionPrice, StudentPriceCurrencySnapshot = assignment.Currency };
